...
class KrogerAllBackupsAccelerator(Accelerator):
    ...
    accelerator_info = """<div class="accelerator">
        ...
        <br>
        <p><strong>Accelerator Info:</strong> To avoid issues, please limit operations to less that 15 Assignment Numbers at a time. This will attempt to process Assignment Numbers in all  formats except for OSD (Overages, Shortages, Damages) - For those queries, use the Kroger OSD Backup accelerator instead.</p>
        <br>
    ...
    async def run(self, invoice_numbers: list[str]) -> str:
        ...
        if '/vendorsso/dashboard' in self.driver.current_url:
            ...
            for invoice_number in remove_duplicates_and_blanks(invoice_numbers):
                if len(str(invoice_number)) > 7:
                    ...
                    if not refresh_token:
                        await self.info("Not Found:{}. Moving to next...".format(invoice_number))
                        continue
            ...
            zip_filename = os.path.join(self.output_path, f"KROG_Backups_Downloader_{count_invoices}_{current_date}")
